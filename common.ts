...
import { readableStreamToString, LuccaTimmiAbsencesError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new LuccaTimmiAbsencesError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new LuccaTimmiAbsencesError(e, e.response?.data)
                }
                if (innerError instanceof LuccaTimmiAbsencesError) {
                    // Got "LuccaTimmiAbsencesError" from the above try block
                    throw innerError;
                }
